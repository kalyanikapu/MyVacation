...
public class Airplane {
	...
	public static int FILE_PARAMETER = 4; //Each passenger has 4 parameters when reading from the fil
	 ...
	 * @return a string that either returns the seat a passenger -
	 * is seated at or that no seats available
	 */
	 ...
	 * @return a String that returns the passenger's name, and where they are seated.
	 * If there is no room at all, the group is not seated 
	 */
	public String add_Group(ArrayList<Passenger> groupToAdd) {
		...
		int maxEmpty = 0; int adjacentEmpty = 0;
			...
			for (a = groupToAdd.size(); a > 0; a--) {
				...
				for (b = 0; b < E_SIZE; b++) {
					...
					if((b % E_COLUMNS == 0) || (b == E_SIZE-1)) {
						...
						maxEmpty = 0; adjacentEmpty = 0; //reset
						list.clear(); //reset
	 ...
	 * @return a String that returns either that the reservation is cancelled
	 * or that the reservation could not be cancelled
	 */
	...
	/**
	 * A method to get all the data saved in the database -
	 * to be outputted to the file 
	 * @return a String with all the flight information
